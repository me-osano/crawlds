...
Singleton {
  ...
  /*
  Shell directories.
  - Default config directory: ~/.config/crawlds
  - Default cache directory: ~/.cache/crawlds
  */
  readonly property alias data: adapter  // Used to access via Settings.data.xxx.yyy
  readonly property int settingsVersion: 53
  property bool isDebug: Quickshell.env("CRAWLDS_DEBUG") === "1"
  readonly property string shellName: "crawlds"
  readonly property string configDir: Quickshell.env("CRAWLDS_CONFIG_DIR") || (Quickshell.env("XDG_CONFIG_HOME") || Quickshell.env("HOME") + "/.config") + "/" + shellName + "/"
  readonly property string cacheDir: Quickshell.env("CRAWLDS_CACHE_DIR") || (Quickshell.env("XDG_CACHE_HOME") || Quickshell.env("HOME") + "/.cache") + "/" + shellName + "/"
  readonly property string settingsFile: Quickshell.env("CRAWLDS_SETTINGS_FILE") || (configDir + "settings.json")
  ...
  JsonAdapter {
    ...
    // core - shared config with daemon (loaded from shell.toml)
    // These control backend services and sync with daemon via DBus
    property JsonObject core: JsonObject {
    }
    ...
    property JsonObject colorSchemes: JsonObject {
      ...
      property string predefinedScheme: "CrawlDS (default)"
