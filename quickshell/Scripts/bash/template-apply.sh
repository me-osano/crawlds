...
kitty)
    ...
    if [ -w "$KITTY_CONF" ]; then
        kitty +kitten themes --reload-in=all crawlds
    else
...
ghostty)
    ...
    if [ -f "$CONFIG_FILE" ]; then
        # Check if theme is already set to crawlds (flexible spacing)
        if grep -qE "^theme\s*=\s*crawlds$" "$CONFIG_FILE"; then
            : # Already correct
        elif grep -qE "^theme\s*=" "$CONFIG_FILE"; then
            # Replace existing theme line in-place
            sed -i -E 's/^theme\s*=.*/theme = crawlds/' "$CONFIG_FILE"
        else
            # Add the new theme line to the end of the file
            echo "theme = crawlds" >>"$CONFIG_FILE"
        fi
...
foot)
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Create the config file with the crawlds theme
        cat >"$CONFIG_FILE" <<'EOF'
[main]
include=~/.config/foot/themes/crawlds
EOF
    else
        # Check if theme is already set to crawlds
        if ! grep -q "include.*crawlds" "$CONFIG_FILE"; then
            ...
            if grep -q '^\[main\]' "$CONFIG_FILE"; then
                ...
                sed -i '/^\[main\]/a include=~/.config/foot/themes/crawlds' "$CONFIG_FILE"
            else
...
alacritty)
    ...
    NEW_THEME_PATH='~/.config/alacritty/themes/crawlds.toml'

    # Check if the config file exists, create it if it doesn't.
    if [ ! -f "$CONFIG_FILE" ]; then
        # Create the config directory if it doesn't exist
        mkdir -p "$(dirname "$CONFIG_FILE")"
        # Create the config file with the crawlds theme import
        cat >"$CONFIG_FILE" <<'EOF'
[general]
import = [
    "~/.config/alacritty/themes/crawlds.toml"
]
EOF
    else
        # Check if crawlds theme is already imported (any path variant)
        if grep -q 'crawlds\.toml' "$CONFIG_FILE"; then
            # Update old relative path to new absolute path if needed
            if grep -q '"themes/crawlds.toml"' "$CONFIG_FILE"; then
                sed -i 's|"themes/crawlds.toml"|"'"$NEW_THEME_PATH"'"|g' "$CONFIG_FILE"
            fi
            # Already has crawlds import with correct path, nothing to do
        else
            # No crawlds import found, add it
...
wezterm)
    ...
    WEZTERM_SCHEME_LINE='config.color_scheme = "CrawlDS"'

    # Check if the config file exists.
    if [ -f "$CONFIG_FILE" ]; then

        # Check if theme is already set to CrawlDS (matches 'CrawlDS' or "CrawlDS")
        if ! grep -q "^\s*config\.color_scheme\s*=\s*['\"]CrawlDS['\"]\s*" "$CONFIG_FILE"; then
            # Not set to CrawlDS. Check if *any* color_scheme line exists.
            if grep -q '^\s*config\.color_scheme\s*=' "$CONFIG_FILE"; then
                # It exists, so we replace it with our desired line.
                sed -i "s|^\(\s*config\.color_scheme\s*=\s*\).*$|\1\"CrawlDS\"|" "$CONFIG_FILE"
            else
...
fuzzel)
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Create the config file with the crawlds theme
        cat >"$CONFIG_FILE" <<'EOF'
include=~/.config/fuzzel/themes/crawlds
EOF
    else
        # Check if theme is already set to crawlds
        if grep -q "^include=~/.config/fuzzel/themes/crawlds$" "$CONFIG_FILE"; then
            : # Already correct
        elif grep -q "^include=.*themes" "$CONFIG_FILE"; then
            # Replace existing theme include line in-place
            sed -i 's|^include=.*themes.*|include=~/.config/fuzzel/themes/crawlds|' "$CONFIG_FILE"
        else
            # Add the new theme include line
            echo "include=~/.config/fuzzel/themes/crawlds" >>"$CONFIG_FILE"
        fi
    fi
    ;;
...
cava)
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        if grep -q '^\[color\]' "$CONFIG_FILE"; then
            # Check if theme is already set to crawlds under [color] (flexible spacing)
            if sed -n '/^\[color\]/,/^\[/p' "$CONFIG_FILE" | grep -qE '^theme\s*=\s*"crawlds"'; then
                : # Already correct
            elif sed -n '/^\[color\]/,/^\[/p' "$CONFIG_FILE" | grep -qE '^theme\s*='; then
                # Replace existing theme line under [color]
                sed -i -E '/^\[color\]/,/^\[/{s/^theme\s*=.*/theme = "crawlds"/}' "$CONFIG_FILE"
                THEME_MODIFIED=true
            else
                # Add theme line after [color]
                sed -i '/^\[color\]/a theme = "crawlds"' "$CONFIG_FILE"
                THEME_MODIFIED=true
            fi
        else
...
EOF
        ...
        if grep -q '^\[flavor\]' "$CONFIG_FILE"; then
            ...
            if sed -n '/^\[flavor\]/,/^\[/p' "$CONFIG_FILE" | grep -q '^dark\s*='; then
                sed -i '/^\[flavor\]/,/^\[/{s/^dark\s*=.*/dark  = "crawlds"/}' "$CONFIG_FILE"
            else
                sed -i '/^\[flavor\]/a dark  = "crawlds"' "$CONFIG_FILE"
            fi
            if sed -n '/^\[flavor\]/,/^\[/p' "$CONFIG_FILE" | grep -q '^light\s*='; then
                sed -i '/^\[flavor\]/,/^\[/{s/^light\s*=.*/light = "crawlds"/}' "$CONFIG_FILE"
            else
                sed -i '/^\[flavor\]/,/^dark/a light = "crawlds"' "$CONFIG_FILE"
            fi
        else
            # Add [flavor] section at the end
            echo "" >>"$CONFIG_FILE"
            echo "[flavor]" >>"$CONFIG_FILE"
            echo 'dark  = "crawlds"' >>"$CONFIG_FILE"
            echo 'light = "crawlds"' >>"$CONFIG_FILE"
        fi
    fi
    ;;
...
niri)
    ...
    INCLUDE_LINE='include "./crawlds.kdl"'
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Check if crawlds include already exists (flexible: quotes, ./ prefix)
        if grep -qE 'include\s+["'"'"'](\./)?crawlds\.kdl["'"'"']' "$CONFIG_FILE"; then
            : # Already included
        else
...
hyprland)
    echo "🎨 Applying 'crawlds' theme to Hyprland..."
    CONFIG_DIR="$HOME/.config/hypr"
    CONFIG_FILE="$CONFIG_DIR/hyprland.conf"
    THEME_FILE="$CONFIG_DIR/crawlds/crawlds-colors.conf"
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        mkdir -p "$(dirname "$CONFIG_FILE")"
        echo -e "\n$INCLUDE_LINE\n" >"$CONFIG_FILE"
        echo "Created new config file with crawlds theme."
    else
        ...
        # Check if crawlds theme source already exists (flexible matching)
        if grep -qE 'source\s*=\s*.*crawlds.*\.conf' "$CONFIG_FILE"; then
            echo "Theme already included, skipping modification."
        else
            # Add the include line to the end of the file
            echo -e "\n$INCLUDE_LINE\n" >>"$CONFIG_FILE"
            echo "✅ Added crawlds theme include to config."
        fi
    fi
...
sway)
    echo "🎨 Applying 'crawlds' theme to Sway..."
    CONFIG_DIR="$HOME/.config/sway"
    CONFIG_FILE="$CONFIG_DIR/config"
    INCLUDE_LINE='include ~/.config/sway/crawlds'
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        # Check if crawlds include already exists (flexible matching)
        if grep -qE 'include\s+.*crawlds' "$CONFIG_FILE"; then
            echo "Theme already included, skipping modification."
        else
            # Add the include line to the end of the file
            echo -e "\n$INCLUDE_LINE\n" >>"$CONFIG_FILE"
...
scroll)
    echo "Applying 'crawlds' theme to Scroll..."
    CONFIG_DIR="$HOME/.config/scroll"
    CONFIG_FILE="$CONFIG_DIR/config"
    INCLUDE_LINE='include ~/.config/scroll/crawlds'
...
mango)
    ...
    THEME_FILE="$CONFIG_DIR/crawlds.conf"
    BACKUP_FILE="$CONFIG_DIR/theme.conf.bak"
    # This sources the crawlds theme file
    ...
    if [ -f "$MAIN_CONFIG" ] && grep -qF "$SOURCE_LINE" "$MAIN_CONFIG"; then
        ...
        if [ -f "$MAIN_CONFIG" ]; then
            echo "" >>"$MAIN_CONFIG"
            echo "# This sources the crawlds theme" >>"$MAIN_CONFIG"
            echo -e "\n$SOURCE_LINE\n" >>"$MAIN_CONFIG"
        else
            echo "# This sources the crawlds theme" >"$MAIN_CONFIG"
            echo -e "\n$SOURCE_LINE\n" >>"$MAIN_CONFIG"
...
btop)
    ...
    if [ -f "$CONFIG_FILE" ]; then
        # Check if theme is already set to crawlds (flexible spacing)
        if grep -qE '^color_theme\s*=\s*"crawlds"' "$CONFIG_FILE"; then
            : # Already correct
        elif grep -qE '^color_theme\s*=' "$CONFIG_FILE"; then
            # Replace existing color_theme line in-place
            sed -i -E 's/^color_theme\s*=.*/color_theme = "crawlds"/' "$CONFIG_FILE"
        else
            echo 'color_theme = "crawlds"' >>"$CONFIG_FILE"
        fi
