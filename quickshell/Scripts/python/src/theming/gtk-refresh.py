...
async def apply_gtk3_colors(config_dir: Path):
    ...
    colors_file = gtk3_dir / "crawlds.css"
    gtk_css = gtk3_dir / "gtk.css"

    if not colors_file.exists():
        print(f"Error: crawlds.css not found at {colors_file}", file=sys.stderr)
        return False
    ...
    gtk_css.symlink_to("crawlds.css")
    print(f"Created symlink: {gtk_css} -> crawlds.css")
    return True


async def apply_gtk4_colors(config_dir: Path):
    gtk4_dir = config_dir / "gtk-4.0"
    colors_file = gtk4_dir / "crawlds.css"
    gtk_css = gtk4_dir / "gtk.css"
    gtk4_import = '@import url("crawlds.css");'

    if not colors_file.exists():
        print(f"Error: GTK4 crawlds.css not found at {colors_file}", file=sys.stderr)
        return False
...
async def get_config_dir() -> Path:
    ...
    if value := os.environ.get("CRAWLDS_CONFIG_DIR"):
