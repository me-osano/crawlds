...
* is rendered in Screen's unified Shape system. This separation prevents
...
PanelWindow {
  ...
  color: "transparent" // Transparent - background is in Screen below
  ...
  WlrLayershell.namespace: "crawlds-bar-content-" + (barWindow.screen?.name || "unknown")
  // Keep bar widgets above Screen's background Shape window when enabled.
  ...
  WlrLayershell.exclusionMode: ExclusionMode.Ignore // Don't reserve space - BarExclusionZone in Screen handles that
