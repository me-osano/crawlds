...
Item {
  ...
  function buildTooltipContent() {
    ...
    // Memory
    rows.push(["Memory", `${Math.round(SystemStatService.memPercent)}% (${SystemStatService.formatGigabytes(SystemStatService.memGb).replace(/[^0-9.]/g, "") + " GB"})`]);

    // Swap (if available)
    if (SystemStatService.swapTotalGb > 0) {
      rows.push(["Swap usage", `${Math.round(SystemStatService.swapPercent)}% (${SystemStatService.formatGigabytes(SystemStatService.swapGb).replace(/[^0-9.]/g, "") + " GB"})`]);
    }
