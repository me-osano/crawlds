...
Item {
    ...
    Item {
      ...
      Loader {
        ...
        sourceComponent: ColumnLayout {
          ...
          Repeater {
            id: repeater
            model: Qt.locale("en").toString(now, formatHorizontal.trim()).split("\\n")
            NText {
              visible: text !== ""
              text: modelData
      ...
      Loader {
        ...
        sourceComponent: ColumnLayout {
          ...
          spacing: -2
          Repeater {
            model: Qt.locale("en").toString(now, formatVertical.trim()).split(" ")
            delegate: NText {
              visible: text !== ""
              text: modelData
              family: useCustomFont && customFont ? customFont : Settings.data.ui.fontDefault
  ...
  function buildTooltipText() {
    if (tooltipFormat && tooltipFormat.trim() !== "") {
      return Qt.locale("en").toString(now, tooltipFormat.trim());
    }
