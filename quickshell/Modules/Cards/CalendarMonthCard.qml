...
NBox {
  ...
  readonly property int firstDayOfWeek: Settings.data.location.firstDayOfWeek === -1 ? Qt.locale("en").firstDayOfWeek : Settings.data.location.firstDayOfWeek
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      NText {
        text: Qt.locale("en").monthName(root.calendarMonth, Locale.LongFormat).toUpperCase() + " " + root.calendarYear
          ...
          Item {
            ...
            NText {
              ...
              text: {
                ...
                const dayName = Qt.locale("en").dayName(dayIndex, Locale.ShortFormat);
                  ...
                  if (events.length > 0) {
                    const summaries = events.map(event => {
                                                   if (root.isAllDayEvent(event)) {
                                                     ...
                                                     const startFormatted = Qt.locale("en").toString(start, timeFormat);
                                                     const end = new Date(event.end * 1000);
                                                     const endFormatted = Qt.locale("en").toString(end, timeFormat);
